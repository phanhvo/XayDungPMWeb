...
exports.getAllAccounts = async () => {
  const result = await db.query(
    "SELECT tentk, phanquyen, trangthai, ma_nhan_vien FROM taikhoan",
  );
  return result.rows;
};
...
exports.getByUsername = async (username) => {
  const result = await db.query("SELECT * FROM taikhoan WHERE tentk = $1", [
    username,
  ]);
...
exports.createAccount = async (data) => {
  ...
  // check trùng
  const check = await db.query("SELECT * FROM taikhoan WHERE tentk = $1", [
    tentk,
  ]);
...
exports.deleteAccount = async (tentk) => {
  await db.query("DELETE FROM taikhoan WHERE tentk = $1", [tentk]);
};
